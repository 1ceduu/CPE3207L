...
\begin{itemize}
  ...
  \item \textbf{Water Availability (Level Sensor):} As depicted in Figure \ref{fig:ultrasonic-sensor}, a waterproof ultrasonic sensor is mounted above each pen's water trough. The ESP32 triggers an ultrasonic pulse and measures the echo time of flight to calculate the distance to the water surface. This determines the current water level and triggers the solenoid valve to refill the trough when the level falls below the required threshold, thereby helping maintain continuous water availability. The complete water-refill control loop is illustrated in Figure \ref{fig:water_refill_flowchart} \cite{whitney2018water,samuel2025water,arguta2024iop}.
    ...
    The water-refill process is illustrated in Figure \ref{fig:water_refill_flowchart}. The system continuously measures the water level; when it drops below the threshold, the solenoid valve opens and the level is monitored until it is restored, at which point the valve closes and the event is logged \cite{whitney2018water,samuel2025water}.
...
\begin{itemize}
  ...
  \item \textbf{Water-level baseline:} The ultrasonic sensor is calibrated by measuring the empty, minimum acceptable, and full trough levels with a manual ruler or dipstick. For validation, the normal operating band is set at 50--100\% of the calibrated trough depth. A refill command is triggered when the level falls below 40\%, and a critical alert is generated when the level remains below 30\% for more than 5 minutes after the refill command or when the level sensor reading deviates by more than $\pm 1$ cm from the reference measurement during calibration.
...
\begin{itemize}
  ...
  \item \textbf{Water level simulation:} The water trough will be filled to a known initial level and recorded as the baseline reading. Water will then be drained in measured amounts to simulate drinking behavior. For abnormal water-level simulation, the water level will be reduced below the configured threshold faster than normal. The system reading before and after draining will be recorded and compared with the expected level to verify whether the automatic refill response and alert trigger operate correctly.
...
\begin{itemize}
  ...
  \item \textbf{Test 3.4 -- User Acceptance Testing (UAT) for UI/UX and System Functionality:} The owner or designated caretaker will complete a UAT checklist covering dashboard navigation, interpretation of green/yellow/red status indicators, viewing pen history, editing a feeding schedule, acknowledging alerts, checking SMS alert content, and using manual override. After completing the checklist, the participant will answer a 5-point Likert-scale form.
